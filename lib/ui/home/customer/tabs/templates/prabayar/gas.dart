...
class _GasPageState extends State<GasPage> with TickerProviderStateMixin {
  ...
  Future<void> _loadData({bool forceRefresh = false}) async {
    ...
    try {
      ...
      if (mounted) {
        setState(() {
          // Filter: Hanya ambil kategori Gas
          _allProducts = products
              .where((p) => p.category.toUpperCase() == "GAS")
              .toList();
          ...
          IconButton(
            ...
            onPressed: () {
              showDialog(
                context: context,
                builder: (context) => AlertDialog(
                  title: const Text('Informasi Gas Prabayar'),
                  content: const SingleChildScrollView(
                    child: Column(
                      ...
                      children: [
                        Text(
                          'Cara Pengisian Gas Prabayar:',
                          style: TextStyle(fontWeight: FontWeight.bold),
                        ),
                        SizedBox(height: 8),
                        Text('1. Masukkan Nomor Pelanggan Gas'),
                        Text('2. Pilih Brand Gas (Pertamina, dll)'),
                        Text('3. Pilih Nominal dan Jumlah'),
                        Text('4. Klik Beli untuk melakukan transaksi'),
                        Text('5. Token akan dikirim setelah pembayaran'),
                        SizedBox(height: 12),
                        Text(
                          'Nomor Pelanggan dapat ditemukan di meteran gas atau kartu pelanggan.',
                          style: TextStyle(
                            fontSize: 12,
    ...
    return Center(
      child: Column(
        ...
        children: [
          ...
          Text(
            'Produk gas belum tersedia saat ini',
  ...
  Widget _buildTabBarView(Color primaryColor) {
    ...
    return TabBarView(
      ...
      children: _dynamicTypes.map((type) {
        final typeProducts = _allProducts.where((p) => p.type == type).toList();
        return Column(
          children: [
          ...
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                if (hasDiscount) const SizedBox(height: 5),
