...
class _DataPageState extends State<DataPage> with TickerProviderStateMixin {
  ...
  Future<void> _loadData({bool forceRefresh = false}) async {
    ...
    try {
      ...
      if (mounted) {
        setState(() {
          // Filter: Hanya ambil kategori DATA
          _allProducts = products
              .where((p) => p.category.toUpperCase().contains("DATA"))
              .toList();
          _isLoading = false;
        });
  ...
  void _updateDynamicTabs() {
    ...
    // Filter berdasarkan Brand dan Kategori DATA
  ...
  Widget _buildProductList(String type) {
    // FILTER AKHIR: Berdasarkan Brand, Category (DATA), dan Type (Tab)
            ...
            children: [
              ...
              if (product.iconUrl != null && product.iconUrl!.isNotEmpty)
                ...
                Container(
                  ...
                  child: Icon(Icons.data_usage, color: appConfig.primaryColor),
                ),
              Expanded(
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
  ...
  Widget _buildEmptyState(Color primaryColor) {
    return Center(
      child: Column(
        ...
        children: [
          Icon(Icons.data_usage, size: 70, color: Colors.grey.shade300),
          const SizedBox(height: 16),
          Text(
            _operatorName.isEmpty
                ? "Masukkan nomor untuk cek produk"
                : "Produk Data tidak ditemukan",
