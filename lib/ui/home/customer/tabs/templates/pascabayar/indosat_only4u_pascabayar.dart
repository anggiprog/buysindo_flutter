...
class IndosatOnly4uPascabayarPage extends StatefulWidget {
  const IndosatOnly4uPascabayarPage({super.key});

  @override
  State<IndosatOnly4uPascabayarPage> createState() =>
      _IndosatOnly4uPascabayarPageState();
}

class _IndosatOnly4uPascabayarPageState
    extends State<IndosatOnly4uPascabayarPage> {
  ...
  Future<void> _cekTagihan() async {
    if (_customerIdController.text.isEmpty) {
      _showSnackbar('Masukkan nomor pelanggan Indosat Only4u', Colors.orange);
      return;
    }
    ...
    try {
      ...
      if (response.statusCode == 200 && response.data != null) {
        ...
        if (products == null || products.isEmpty) {
          _showSnackbar('Produk Indosat Only4u tidak ditemukan.', Colors.red);
          return;
        }
        // Cari produk Indosat Only4u (brand == 'Indosat Only4u')
        final indosatProduct = products.firstWhere(
          (p) => (p['brand']?.toString().toLowerCase() == 'indosat only4u'),
          orElse: () => null,
        );
        if (indosatProduct == null) {
          _showSnackbar(
            'Produk Indosat Only4u tidak tersedia untuk akun Anda.',
            Colors.red,
          );
          return;
        }
        ...
        await CekTagihanPascabayar.showCekTagihan(
          context: context,
          productName:
              indosatProduct['product_name'] ?? 'Indosat Only4u Pascabayar',
          brand: indosatProduct['brand'] ?? 'Indosat Only4u',
                  ...
                  Expanded(
                    child: Column(
                      ...
                      children: [
                        ...
                        Text(
                          'Bayar tagihan Indosat Only4u Anda',
                          style: TextStyle(
