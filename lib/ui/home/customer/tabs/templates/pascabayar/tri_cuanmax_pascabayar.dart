...
class TriCuanMaxPascabayarPage extends StatefulWidget {
  const TriCuanMaxPascabayarPage({super.key});

  @override
  State<TriCuanMaxPascabayarPage> createState() =>
      _TriCuanMaxPascabayarPageState();
}

class _TriCuanMaxPascabayarPageState extends State<TriCuanMaxPascabayarPage> {
  ...
  Future<void> _cekTagihan() async {
    if (_customerIdController.text.isEmpty) {
      _showSnackbar('Masukkan nomor pelanggan Tri CuanMax', Colors.orange);
      return;
    }
    ...
    try {
      ...
      if (response.statusCode == 200 && response.data != null) {
        ...
        if (products == null || products.isEmpty) {
          _showSnackbar('Produk Tri CuanMax tidak ditemukan.', Colors.red);
          return;
        }
        // Cari produk Tri CuanMax (brand == 'Tri CuanMax')
        final triProduct = products.firstWhere(
          (p) => (p['brand']?.toString().toLowerCase() == 'tri cuanmax'),
          orElse: () => null,
        );
        if (triProduct == null) {
          _showSnackbar(
            'Produk Tri CuanMax tidak tersedia untuk akun Anda.',
            Colors.red,
          );
          return;
        }
        ...
        await CekTagihanPascabayar.showCekTagihan(
          ...
          brand: triProduct['brand'] ?? 'Tri CuanMax',
                  ...
                  Expanded(
                    child: Column(
                      ...
                      children: [
                        ...
                        Text(
                          'Bayar tagihan Tri CuanMax Anda',
                          style: TextStyle(
