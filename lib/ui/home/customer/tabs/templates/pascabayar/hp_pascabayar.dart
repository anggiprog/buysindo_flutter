...
class HpPascabayar extends StatefulWidget {
  const HpPascabayar({super.key});

  @override
  State<HpPascabayar> createState() => _HpPascabayarState();
}

class _HpPascabayarState extends State<HpPascabayar> {
  ...
  static const String _cacheKey = 'hp_products_cache';
  ...
  Future<void> _loadProducts({bool forceRefresh = false}) async {
    print('🔄 [HP] _loadProducts called (forceRefresh: $forceRefresh)');
    ...
    try {
      ...
      print('🔑 [HP] Token: ${token?.substring(0, 20)}...');
      ...
      if (response.statusCode == 200) {
        ...
        // Filter hanya produk HP PASCABAYAR
        final hpProducts = productResponse.products
            .where(
              (p) =>
                  p.brand.toUpperCase().contains('HP') &&
  ...
  Future<void> _refreshProducts() async {
    ...
    try {
      ...
      if (mounted) {
        _showSnackbar('Data HP berhasil diperbarui', Colors.green);
      }
    } finally {
      if (mounted) {
        setState(() {
