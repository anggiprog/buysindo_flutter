...
class ByuPascabayarPage extends StatefulWidget {
  const ByuPascabayarPage({super.key});

  @override
  State<ByuPascabayarPage> createState() => _ByuPascabayarPageState();
}

class _ByuPascabayarPageState extends State<ByuPascabayarPage> {
  ...
  Future<void> _cekTagihan() async {
    if (_customerIdController.text.isEmpty) {
      _showSnackbar('Masukkan nomor pelanggan by.U', Colors.orange);
      return;
    }
    ...
    try {
      ...
      if (response.statusCode == 200 && response.data != null) {
        ...
        if (products == null || products.isEmpty) {
          _showSnackbar('Produk by.U tidak ditemukan.', Colors.red);
          return;
        }
        // Cari produk by.U (brand == 'by.U' atau buyer_sku_code == 'byu')
        final byuProduct = products.firstWhere(
          (p) =>
              (p['brand']?.toString().toLowerCase() == 'by.u') ||
              (p['buyer_sku_code']?.toString().toLowerCase() == 'byu'),
          orElse: () => null,
        );
        if (byuProduct == null) {
          _showSnackbar(
            'Produk by.U tidak tersedia untuk akun Anda.',
            Colors.red,
          );
          return;
        }
        ...
        await CekTagihanPascabayar.showCekTagihan(
          context: context,
          productName: byuProduct['product_name'] ?? 'by.U Pascabayar',
          brand: byuProduct['brand'] ?? 'by.U',
