...
class TelkomselOmniPascabayarPage extends StatefulWidget {
  const TelkomselOmniPascabayarPage({super.key});

  @override
  State<TelkomselOmniPascabayarPage> createState() =>
      _TelkomselOmniPascabayarPageState();
}

class _TelkomselOmniPascabayarPageState
    extends State<TelkomselOmniPascabayarPage> {
  ...
  Future<void> _cekTagihan() async {
    if (_customerIdController.text.isEmpty) {
      _showSnackbar('Masukkan nomor pelanggan Telkomsel Omni', Colors.orange);
      return;
    }
    ...
    try {
      ...
      if (response.statusCode == 200 && response.data != null) {
        ...
        if (products == null || products.isEmpty) {
          _showSnackbar('Produk Telkomsel Omni tidak ditemukan.', Colors.red);
          return;
        }
        // Cari produk Telkomsel Omni (brand == 'Telkomsel Omni')
        final telkomselOmniProduct = products.firstWhere(
          (p) => (p['brand']?.toString().toLowerCase() == 'telkomsel omni'),
          orElse: () => null,
        );
        if (telkomselOmniProduct == null) {
          _showSnackbar(
            'Produk Telkomsel Omni tidak tersedia untuk akun Anda.',
            Colors.red,
          );
          return;
        }
        ...
        await CekTagihanPascabayar.showCekTagihan(
          context: context,
          productName:
              telkomselOmniProduct['product_name'] ??
              'Telkomsel Omni Pascabayar',
          brand: telkomselOmniProduct['brand'] ?? 'Telkomsel Omni',
                  ...
                  Expanded(
                    child: Column(
                      ...
                      children: [
                        ...
                        Text(
                          'Bayar tagihan Telkomsel Omni Anda',
                          style: TextStyle(
